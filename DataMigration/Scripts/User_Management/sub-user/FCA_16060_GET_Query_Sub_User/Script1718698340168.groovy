...
//    def tokenQAMIG = CustomKeywords.'common.Utility.getValueFromResponseJson'(tokenResponseQAMIG, 'access_token')
    ...
    ResponseObject queryResponseQA = WS.sendRequest(findTestObject('CommonRequest/GET_Method_QA', [('token') : GlobalVariable.airline450QA, ('endPoint') : path]))
    ...
    ResponseObject queryResponseQAMIG = WS.sendRequest(findTestObject('CommonRequest/GET_Method_QA_MIG', [('token') : GlobalVariable.airline450QAMIG, ('endPoint') : path]))
...
//				}
				...
				if(id1s.size() != id2s.size()) {
					...
					if(JSONArrayQA.length()!=JSONArrayQAMIG.length() && !missingIds.contains(ignoreRecordHavingKey)) {
						KeywordUtil.markFailed("There are "+JSONArrayQA.length()+" RECORDS in QA and "+JSONArrayQAMIG.length()+" RECORDS in QAMIG")
					}
