	...
	if(jsonObjectQA.has("records") || jsonObjectQAMIG.has("records")) {
		...
		jsonObjectQA.keySet().each { key ->
			...
			if(!(jsonObjectQA.get(key) instanceof JSONArray)) {
				...
				HashMap<String,JSONObject> hashMapQA = new LinkedHashMap<>();
				ArrayList<String> criteria1List = new ArrayList<>();
				ArrayList<String> criteria2List = new ArrayList<>();
				ArrayList<String> criteria3List = new ArrayList<>();
				for (int i = 0; i < JSONArrayQA.length(); i++) {
					criteria1List.add(String.valueOf(JSONArrayQA.getJSONObject(i).get("evaluationFileId")));
					criteria2List.add(String.valueOf(JSONArrayQA.getJSONObject(i).get("uploadDate")));
					criteria3List.add(String.valueOf(JSONArrayQA.getJSONObject(i).get("adma")));
					hashMapQA.put(String.valueOf(i),JSONArrayQA.getJSONObject(i));
				}
				
				HashMap<String,JSONObject> hashMapQAMIG = new LinkedHashMap<>();
				for (int listI = 0; listI < criteria1List.size(); listI++) {
					for (int i = 0; i < JSONArrayQAMIG.length(); i++) {
						String criteria1 = String.valueOf(JSONArrayQAMIG.getJSONObject(i).get("evaluationFileId"));
						String criteria2 = String.valueOf(JSONArrayQAMIG.getJSONObject(i).get("uploadDate"));
						String criteria3 = String.valueOf(JSONArrayQAMIG.getJSONObject(i).get("adma"));
						if(criteria1List.get(listI).equals(criteria1) && criteria2List.get(listI).equals(criteria2) && criteria3List.get(listI).equals(criteria3)){
							hashMapQAMIG.put(String.valueOf(listI),JSONArrayQAMIG.getJSONObject(i));
							break;
						}
					}
				}
				
				String rQA = new JSONObject(hashMapQA).toString();
				String rQAMIG = new JSONObject(hashMapQAMIG).toString();
				
//				String ignoreNodes = "$.*.id";
				String[] strings = ignoreNodes.split(",");
				
			try {
				JsonAssertions.assertThatJson(rQA).whenIgnoringPaths("\$.*.id").isEqualTo(rQAMIG);
//						.withDifferenceListener((difference, context) -> {
//							System.out.println("difference.getActualPath() = " + difference.getActualPath());
//							System.out.println("Actual = " + difference.getActual());
//							System.out.println("difference.getExpectedPath() = " + difference.getExpectedPath());
//							System.out.println("Expect = " + difference.getExpected());
//							System.out.println("---------------------------------------------------------");
//						})
