		...
		jsonObjectQA.keySet().each { key ->
			...
			if(!(jsonObjectQA.get(key) instanceof JSONArray)) {
				...
				if(JSONArrayQA.length()!=JSONArrayQAMIG.length()) {
					KeywordUtil.markFailedAndStop("There are "+JSONArrayQA.length()+" RECORDS in QA and "+JSONArrayQAMIG.length()+" RECORDS in QAMIG")
				}
				
				Set<String> dateSQA = new LinkedHashSet();
				
				for(int i = 0; i<JSONArrayQA.length(); i++) {
					String o  = (String)JSONArrayQA.get(i).getAt("name")
					dateSQA.add(o)
				}
				
				Set<String> dateSQAMIG = new LinkedHashSet();
				
				for(int i = 0; i<JSONArrayQAMIG.length(); i++) {
					String o  = (String)JSONArrayQAMIG.get(i).getAt("name")
					dateSQAMIG.add(o)
				}
				
				List<String> dateQA = new ArrayList(dateSQA);
				
				for(int dateI = 0; dateI<dateQA.size(); dateI++) {
					List<JSONObject> listOfJsonQA = new ArrayList();
					List<JSONObject> listOfJsonQAMIG = new ArrayList();
				
					for(int arrayIndex = 0; arrayIndex<JSONArrayQA.length(); arrayIndex++) {
						String dateArrayQA  = (String)JSONArrayQA.get(arrayIndex).getAt("name")
						if(dateArrayQA.equals(dateQA.get(dateI))) {
							listOfJsonQA.add(JSONArrayQA.get(arrayIndex))
						}
						
						String dateArrayQAMIG  = (String)JSONArrayQAMIG.get(arrayIndex).getAt("name")
						if(dateArrayQAMIG.equals(dateQA.get(dateI))) {
							listOfJsonQAMIG.add(JSONArrayQAMIG.get(arrayIndex))
						}
					}
					
					JSONArray jAQA = new JSONArray(listOfJsonQA)
					JSONArray jAQAMIG = new JSONArray(listOfJsonQAMIG)
					
					Object jsonQA = jsonSlurper.parseText(jAQA.toString())
					
					Object jsonQAMIG = jsonSlurper.parseText(jAQAMIG.toString())
					...
					KeywordUtil.logInfo("qaA    "+qaA.toString())
					KeywordUtil.logInfo("qamigA "+qamigA.toString())
					KeywordUtil.logInfo("==============================")
