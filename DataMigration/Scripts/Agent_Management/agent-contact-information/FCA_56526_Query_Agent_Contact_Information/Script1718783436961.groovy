...
//    def tokenQAMIG = CustomKeywords.'common.Utility.getValueFromResponseJson'(tokenResponseQAMIG, 'access_token')
    ...
    ResponseObject queryResponseQA = WS.sendRequest(findTestObject('CommonRequest/GET_Method_QA', [('token') : GlobalVariable.agent73786127QA, ('endPoint') : path]))
    ...
    ResponseObject queryResponseQAMIG = WS.sendRequest(findTestObject('CommonRequest/GET_Method_QA_MIG', [('token') : GlobalVariable.agent73786127QAMIG
                , ('endPoint') : path]))
...
//		boolean check = true;
		jsonObjectQA.keySet().each { key ->
			...
			if(jsonObjectQA.get(key) instanceof JSONObject) {
				JSONObject qaJ = jsonObjectQA.get(key);
				JSONObject qamigJ = jsonObjectQAMIG.get(key);
//				KeywordUtil.logInfo("JSON class is "+jsonObjectQA.get(key).getClass())
				qaJ.keySet().each { key1 ->
					KeywordUtil.logInfo("KEY1 for json "+key1)
					if(!ignoreNodesList.contains(key+"."+key1)) {
//						KeywordUtil.logInfo("value for qa json "+qaJ.get(key1))
//						KeywordUtil.logInfo("value for qamig json "+qamigJ.get(key1))
//						KeywordUtil.logInfo("S---------")
//						KeywordUtil.logInfo(key+"."+key1+ "---------> key+.+key1+")
//						KeywordUtil.logInfo("---------")
//						if(qaJ.get(key1) instanceof String) {
//							KeywordUtil.logInfo(key+"."+key1+ "---------> key+.+key1+")
//						}
//						KeywordUtil.logInfo("class is "+qaJ.get(key1).getClass())
						String qaS = "{"+key+"."+key1+":\""+qaJ.get(key1)+"\"}";
						String qamigS = "{"+key+"."+key1+":\""+qamigJ.get(key1)+"\"}";
//						KeywordUtil.logInfo("value for qa string "+qaS)
//						KeywordUtil.logInfo("value for qamig string "+qamigS)
//						KeywordUtil.logInfo("---------")
//						KeywordUtil.logInfo("length for qa string "+qaS.length())
//						KeywordUtil.logInfo("length for qamig string "+qamigS.length())
...
//						.replaceAll("(", "").replaceAll(")","")
				...
				String qaS = "{"+key+":\""+jsonObjectQA.get(key)+"\"}";
				String qamigS = "{"+key+":\""+jsonObjectQAMIG.get(key)+"\"}";
			...
			KeywordUtil.logInfo("---------")
//			JSONCompareResult compareJSON = compareJSON(jsonObjectQA.get(key), jsonObjectQAMIG.get(key), comparator)
//			CustomKeywords.'compare.GetDifferences.setDifferencesFromResult'(compareJSON)
		}
