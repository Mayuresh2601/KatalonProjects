...
//    def tokenQAMIG = CustomKeywords.'common.Utility.getValueFromResponseJson'(tokenResponseQAMIG, 'access_token')
    ...
    ResponseObject queryResponseQA = WS.sendRequest(findTestObject('CommonRequest/GET_Method_QA', [('token') : GlobalVariable.airlineQA_191_67724465, ('endPoint') : path]))
    ...
    ResponseObject queryResponseQAMIG = WS.sendRequest(findTestObject('CommonRequest/GET_Method_QA_MIG', [('token') : GlobalVariable.airlineQAMIG_191_67724465
                , ('endPoint') : path]))
		...
		jsonObjectQA.keySet().each { key ->
			...
			if(!(jsonObjectQA.get(key) instanceof JSONArray)) {
				...
				ignoreNodesList.each { ignoreNode ->
					...
					if(ignoreNode.contains("records")) {
