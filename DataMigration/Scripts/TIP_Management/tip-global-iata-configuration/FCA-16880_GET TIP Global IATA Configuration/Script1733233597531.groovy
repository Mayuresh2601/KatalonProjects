...
//    def tokenQAMIG = CustomKeywords.'common.Utility.getValueFromResponseJson'(tokenResponseQAMIG, 'access_token')
	...
	JSONObject jsonQAMigResponse = CustomKeywords.'compare.CompareJsonResponse.convertJsonValueToString'(jsonObjectQAMIG, StringConvertedKeys)
	...
	if(jsonObjectQA.has("parameters") || jsonObjectQAMIG.has("parameters")) {
		...
		ignoreNodesList.each { node ->
			...
			CustomKeywords.'compare.CompareJsonResponse.removeNode'(jsonQAMigResponse, node)
		}
		JSONCompareResult compareJSON = compareJSON(jsonObjectQA, jsonQAMigResponse, comparator)
