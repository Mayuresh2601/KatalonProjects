...
//    def tokenQAMIG = CustomKeywords.'common.Utility.getValueFromResponseJson'(tokenResponseQAMIG, 'access_token')
    ...
    ResponseObject queryResponseQA = WS.sendRequest(findTestObject('CommonRequest/GET_Method_QA_Spanish', [('token') : GlobalVariable.airlineQA_026_6772279, ('endPoint') : path]))
    ...
    ResponseObject queryResponseQAMIG = WS.sendRequest(findTestObject('CommonRequest/GET_Method_QA_MIG_Spanish', [('token') : GlobalVariable.airlineQAMIG_026_6772279, ('endPoint') : path]))
	...
	CustomKeywords.'compare.GetDifferences.clearSetValues'()
		...
		jsonObjectQAMIG.keySet().each { key ->
			
			if(!(jsonObjectQAMIG.get(key) instanceof JSONArray)) {
				
				if(!ignoreNodesList.contains(key)) {
					KeywordUtil.logInfo("Key comparing "+key)
