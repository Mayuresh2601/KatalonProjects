...
//    def tokenQAMIG = CustomKeywords.'common.Utility.getValueFromResponseJson'(tokenResponseQAMIG, 'access_token')
    ...
    ResponseObject queryResponseQA = WS.sendRequest(findTestObject('CommonRequest/GET_Method_QA', [('token') : GlobalVariable.airlineQA73782113, ('endPoint') : path]))
    ...
    ResponseObject queryResponseQAMIG = WS.sendRequest(findTestObject('CommonRequest/GET_Method_QA_MIG', [('token') : GlobalVariable.airlineQAMIG73782113             , ('endPoint') : path]))
	...
	if(jsonObjectQA.has("records") || jsonObjectQAMIG.has("records")) {
		...
		jsonObjectQA.keySet().each { key ->
			...
			if(!(jsonObjectQA.get(key) instanceof JSONArray)) {
				...
				if(JSONArrayQA.length()==0 && JSONArrayQAMIG.length()==0) {
					KeywordUtil.markWarning("There are 0 RECORDS in the Reponse from QA and QAMIG")
				} else if(JSONArrayQA.length()==0) {
				...
				KeywordUtil.logInfo("names QA "+id1s)
				KeywordUtil.logInfo("names QA mig "+id2s)
				...
				JSONCompareResult compareResult = compareJSON(qaA, qamigA, JSONCompareMode.NON_EXTENSIBLE)
...
//				JSONCompareResult compareResult = CustomKeywords.'compare.CompareJsonResponse.compareObjectValuesInArray'(keyToSort, qaA, qamigA)
	...
	KeywordUtil.logInfo("failed size "+failedList.size())
	KeywordUtil.logInfo("missing size "+missingList.size())
	KeywordUtil.logInfo("unexpected size "+unexpectedList.size())
