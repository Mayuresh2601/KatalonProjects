	...
	if(jsonObjectQA.has("records") || jsonObjectQAMIG.has("records")) {
		...
		jsonObjectQA.keySet().each { key ->
			...
			if(!(jsonObjectQA.get(key) instanceof JSONArray)) {
				...
				Set<String> uniqueKeyQA = new LinkedHashSet();
				
				for(int i = 0; i<JSONArrayQA.length(); i++) {
					String o  = (String)JSONArrayQA.get(i).getAt(keyUnique)
					uniqueKeyQA.add(o)
				}
				
				Set<String> uniqueKeyQAMIG = new LinkedHashSet();
				
				for(int i = 0; i<JSONArrayQAMIG.length(); i++) {
					String o  = (String)JSONArrayQAMIG.get(i).getAt(keyUnique)
					uniqueKeyQAMIG.add(o)
				}
